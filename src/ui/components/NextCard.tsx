...
export function NextCard({ variant, boardLength, players, equity, used, advanced, focus, onFocus, onDeal }: Props) {
  const street = boardLength === 3 ? 'turn' : boardLength === 4 ? 'river' : undefined
  if (!street) return null

  const who = advanced ? focus : 0
  const whoLabel = who === 0 ? 'you' : playerLabel(who)
  const minRank = VARIANTS[variant].minRank
  const ranks: number[] = []
  for (let r = 12; r >= minRank; r--) ranks.push(r)
  const base = equity.result?.players[who]?.equity
  ...
  for (const [, eq] of results) {
    ...
    if (d > 0.005) improve++
    if (d < -0.005) worsen++
    if (eq[who] === Math.max(...eq) && eq[who] > 0) ahead++
  }
  const done = equity.nextStatus === 'done'

  return (
    <section className="nextcard" aria-labelledby="nextcard-title">
      <div className="section-head">
        <h2 id="nextcard-title" className="section-title">
          Which {street} cards help {who === 0 ? 'you' : playerLabel(who)}?
        </h2>
        {advanced && players.length > 1 && (
          <div className="segmented" role="group" aria-label="Show cards for">
      ...
      <p className="nextcard-summary" aria-live="polite">
        {possible === 0 ? (
          <span className="muted">Checking every possible {street} card…</span>
        ) : (
          <>
            <strong className="is-better mono">{improve}</strong> cards improve {who === 0 ? 'your' : 'their'} chances,{' '}
            <strong className="is-worse mono">{worsen}</strong> hurt them.
            {players.length > 1 && (
              <>
                {' '}
                After <strong className="mono">{ahead}</strong> of {possible}, {whoLabel} {who === 0 ? 'are' : 'is'} the favourite.
              </>
            )}
            {!done && <span className="muted"> ({results.size} of {equity.nextTotal} checked)</span>}
          </>
        )}
      </p>

      <div className="nextcard-grid" role="grid" aria-label={`Chance to win after each ${street} card`} style={{ gridTemplateColumns: `auto repeat(${ranks.length}, 1fr)` }}>
            ...
            {ranks.map((rank) => {
              ...
              return (
                <button
                  ...
                  aria-label={
                    owner
                      ? `${cardName(card)} is already in play`
                      : value !== undefined
                        ? `${cardName(card)}: ${(value * 100).toFixed(1)}% to win, ${delta! >= 0 ? 'up' : 'down'} ${Math.abs(delta!).toFixed(1)} points. Deal it.`
                        : `${cardName(card)}: checking`
                  }
                  title={value !== undefined ? `${cardName(card)}: ${(value * 100).toFixed(1)}% (${delta! >= 0 ? '+' : ''}${delta!.toFixed(1)})` : undefined}
                >
                  {owner ? <span className="nextcard-owner">{owner === 'Board' ? 'B' : owner === 'Dead' ? 'D' : owner === 'You' ? 'Y' : owner.replace('Opp ', 'O')}</span> : value !== undefined ? <span className="mono">{Math.round(value * 100)}</span> : <span className="nextcard-wait" aria-hidden="true" />}
                </button>
              )
            })}
          </div>
      ...
      <p className="section-foot">
        Each number is {who === 0 ? 'your' : `${playerLabel(who)}'s`} chance to win if that card comes next. Green helps, red hurts. Tap a card to deal it.
      </p>
    </section>
  )
}
