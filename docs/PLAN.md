...
Status: shipped. Research, stress testing, engine, UI, SEO, CI and deployment are complete; this document records the decisions.
...
Performance follow-up, adopted after benchmarking: OMPEval-style lookup scorer and direct Omaha evaluation (identical values, 3 to 40x faster), irrelevant-suit board merging and a suit-canonical deal cache for exact mode, and a node-budgeted joint range search. Heads-up preflop exact dropped from 174 ms to about 35 ms, AK vs 22 from 17.7 s to about 100 ms, PLO4 heads-up preflop from 2.7 s to about 110 ms.
...
Should have (shipped)
- Random opponents (new players start as random hands), range presets and percent windows.
- Random flop, turn and river dealing, reorder players, clear per section, undo and redo.

Next (not in this release)
- Named ranges saved in localStorage.
- Street timeline: hero equity preflop, flop, turn and river for a given board.
...
Worker protocol: `equity {id, request}` produces a `plan`, `progress` snapshots and one `done` or `error`. Monte Carlo runs in short slices and stops on a `stop` message so workers stay warm; synchronous jobs are cancelled by terminating the worker. Monte Carlo shards run with distinct seeds; the pool merges raw accumulators (weights, equity sums, squared sums, category counts) so the merged stderr is exact.

Preflop orderings: generated by `scripts/gen-preflop-ranking.ts` with the engine itself (exact against one random hand, 20M seeded Monte Carlo trials against three), for Hold'em and Short Deck, vs 1 and vs 3 random hands. Committed as data with the generator.
...
Smoky lounge after hours: smoke-tinted near black, ember orange for the hero, cobalt for interaction, green and red only for better and worse. Fraunces for display, Inter Tight for UI, JetBrains Mono for every number. Four-colour suits. Cards drawn in CSS, crisp at any size. Motion limited to progress and value transitions.
